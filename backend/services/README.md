...
and lets us validate each service boundary (auth done & verified) before the
next.
...
- **monolith** (`server/`) — still owns services, bookings, dispatch/realtime,
  payments, wallet, support, CMS, admin.

## Next services to extract (priority order)
1. **payment-service** — payments + wallet + withdrawals (clear boundary, PCI isolation).
2. **booking-service** — bookings + reviews (the core domain).
3. **dispatch/realtime-service** — Socket.IO + dispatch engine + BullMQ worker.
4. **notification-service** — notifications + FCM.
